...
def make_sidebar(df):
    ...
    countries = st.sidebar.multiselect(
        "Choose the Countries to view Informations",
...
def main():
    st.set_page_config(page_title="Cuisines", page_icon=":fork_and_knife:", layout="wide")

    df = cdt.read_processed_data()

    countries, top_n, cuisines = make_sidebar(df)

    st.markdown("# :knife_fork_plate: Types of Cuisine")

    df_restaurants = cdt.top_restaurants(countries, cuisines, top_n)

    st.markdown(f"## Best restaurants of different types of cuisine")
