...
def top_cuisines():
    ...
    for key in cuisines.keys():
        ...
        cuisines[key] = (df.loc[lines, cols].sort_values(["aggregate_rating", "restaurant_id"], ascending=[False, True]).iloc[0, :].to_dict())
...
def top_best_cuisines(countries, top_n):
    ...
    grouped_df = (df.loc[lines, ["aggregate_rating", "cuisines"]].groupby("cuisines").mean().sort_values("aggregate_rating", ascending=False).reset_index().head(top_n))

    fig = px.bar(grouped_df.head(top_n),x="cuisines",y="aggregate_rating",text="aggregate_rating",text_auto=".2f",title=f"Top {top_n} best types of culinaries",labels={"cuisines": "Types of culinaries","aggregate_rating": "Aggregate rating",},)
...
def top_worst_cuisines(countries, top_n):
    ...
    grouped_df = ( df.loc[lines, ["aggregate_rating", "cuisines"]].groupby("cuisines").mean().sort_values("aggregate_rating").reset_index().head(top_n))

    fig = px.bar(grouped_df.head(top_n),x="cuisines",y="aggregate_rating",text="aggregate_rating",text_auto=".2f",title=f"Top {top_n} worst types of culinaries",labels={"cuisines": "Types of culinaries","aggregate_rating": "Aggregate rating",},)
